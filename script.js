...
function animateScrollBlueBar() {
  ...
  const scrollProgress = Math.min((scrollTop / scrollHeight) * 1, 1);
...
function updateTimelineProgress() {
  ...
  const visible = Math.min(
    Math.max(
      ((windowHeight - rect.top) / (rect.height + windowHeight)) * 1.5,
      0
    ),
    1
  );
